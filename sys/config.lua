...
local function MigrateData()
    ...
    elseif IsNewConfigVersion() then
        ...
        Addon:Debug("default", "Rules config has been migrated.")
    else
        ...
        notifyChanges =
            function(self)
                if ((self.suspend == 0) and self.changes) then
                    Addon:Debug("default", "Notifying Changes");
                    for _, callback in ipairs(self.handlers) do
                        local status, result = pcall(callback, self)
                        if (not status) then
                            Addon:Debug("default", "Config: Changed callback failed: \"%s%s%s\"", RED_FONT_COLOR_CODE, result, FONT_COLOR_CODE_CLOSE)
                        end
                    end
                    self.changes = false
                end
            ...
            if (not self.ensured) then
                ...
                if (not Vendor_Settings) then
                    ...
                    Addon:Debug("default", "Settings have been set from defaults.")
                elseif (Vendor_Settings and (Vendor_Settings.version ~= Addon.DefaultConfig.Settings.version)) then
                    ...
                    Addon:Debug("default", "Settings have been migrated.")
                end

                -- If rules config doesn't exist, initialize it with the defaults.
                if (not Vendor_RulesConfig) then
                    Vendor_RulesConfig = Addon.DeepTableCopy(Addon.DefaultConfig.Rules)
                    Addon:Debug("default", "Rules config has been set from defaults.")
...
function Addon.Config:migrateSettings(oldSettings, newSettings)
    Addon:Debug("default", "[Config]: +Begin migrating settings from v=%d to v=%d", oldSettings.version, newSettings.version)
    -- Migrate the sell_never to the new version
    if (rawget(oldSettings, NEVER_SELL)) then
        Addon:Debug("default", "[Config]: |         Copying never sell list with %d items", #rawget(oldSettings, NEVER_SELL))
        rawset(newSettings, NEVER_SELL, rawget(oldSettings, NEVER_SELL))
    end

    if (rawget(oldSettings, ALWAYS_SELL)) then
        Addon:Debug("default", "[Config]: |         Copying the always sell list %d items", #rawget(oldSettings, ALWAYS_SELL))
        rawset(newSettings, ALWAYS_SELL, rawget(oldSettings, ALWAYS_SELL))
    end
    Addon:Debug("default", "[Config] +Setting migration complete")
end
...
function Addon.Config:migrateRulesConfig(oldSettings, newSettings)
    Addon:Debug("default", "[Config]: +Begin migrating rules from v=%d to v=%d", oldSettings.version, newSettings.version)

    if (oldSettings.version == 3) and (newSettings.version == 4) then
        if (rawget(oldSettings, KEEP_RULES)) then
            Addon:Debug("default", "[Config]: |         Copying keep rules with with %d items", #rawget(oldSettings, KEEP_RULES))
            rawset(newSettings, KEEP_RULES, rawget(oldSettings, KEEP_RULES))
            Addon:Debug("default", "[Config]: |         adding equipmentset keep rule");
            table.insert(rawget(newSettings, KEEP_RULES), "equipmentset");
        end

        if (rawget(oldSettings, SELL_RULES)) then
            Addon:Debug("default", "[Config]: |         Copying sell rules with with %d items", #rawget(oldSettings, SELL_RULES))
            rawset(newSettings, SELL_RULES, rawget(oldSettings, SELL_RULES))
        end
    end

    if (oldSettings.version < 5) then
        Addon:Debug("default", "[Config]: |         Need to migrate rule ids")
    ...
    Addon:Debug("default", "[Config] +Rules migration complete");
end
